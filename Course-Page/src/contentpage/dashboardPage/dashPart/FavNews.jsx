...
import ShowFavNewsDetail from "../../../component/dashComponent/ShowFavNewsDetail"
...
const FavNews = () => {
 ...
 const [filteredData, setFilteredData] = useState([]);
  ...
  const [showCourseDetail, setShowCourseDetail] = useState(null); 
   ...
   const SortCourse = [...courseData].sort((a, b) => {
     if (filters.sortBy === "جدید ترین") return  new Date(b?.news?.insertDate) -  new Date(a?.news?.insertDate);
  ...
  const fetchProduct = async () => {
    try {
      ...
      setCourseData(response.data.myFavoriteNews  || []);
       setFilteredData(response.data.myFavoriteNews || []);
    } catch (error) {
 ...
 const deleteFavNews = async (ResId)=>{
         try{
            const delte = await apiClient.delete("/News/DeleteFavoriteNews",{
              data: { 
                deleteEntityId: String(ResId)
              }
            });
            toast.success("اخبار از لیست مورد علاقه حذف شد")
  ...
  const handleSearch = (value) => {
    ...
    const teacheName = item.auther?.toLowerCase() || '';
     ...
     {showCourseDetail && (
      ...
      <div className="bg-white w-full h-[85%] rounded-b-[25px]">
        ...
        <div className="w-full bg-[#F5F5F5] h-[40px] font-medium lg:text-[15px] text-[10px] pb-[10px] text-gray-600 hidden lg:flex flex-row">
                 <div className="h-full flex items-center mr-2 w-[40%]">عکس و نام اخبار</div>
                 <div className="h-full flex items-center w-[15%]">نام نویسنده</div>
                 <div className="h-full flex items-center w-[15%]">تاریخ شروع</div>
                 <div className="h-full flex items-center w-[15%]">تاریخ پایان</div>
                 <div className="h-full flex items-center w-[15%]">لینک اخبار</div>
               </div>
               ...
               <div className="commentContent w-full font-medium whitespace-nowrap flex flex-col rounded-b-[25px]">
                       ...
                       >
                           ...
                           <div className="h-full ml-3 lg:ml-0 w-[30%] lg:w-[39%] mr-3 flex flex-row items-center gap-5">
                           <img
                           onError={(e) => {e.currentTarget.onerror = null ;e.currentTarget.src ="/cover3.png"}} 
                               src={card.currentImageAddressTumb || "/cover3.png"}
                               className="rounded-[15px] h-[80%] w-full lg:w-[20%]"
                           />
                           ...
                           <span className="w-[33%] lg:w-[33%]">
                               {card.auther}
                           </span>
       
                          
                           <span className="w-[33%] lg:w-[30%] hidden lg:block">
                               {card?.news?.insertDate?.slice(0, 10)}
                           </span>
       
                           
                           <span className="w-[33%] lg:w-[30%] hidden lg:block">
                               {card?.news?.updateDate?.slice(0, 10)}
                           </span>
       
                           
                           <span className="w-[33%] lg:w-[5%]  text-black">
                               <span className="text-gray-500 lg:inline hidden">{card?.news?.shortLink || "؟"}</span>
                               ...
                               {card?.accept ? "تایید شده": "در انتظار تایید"}
                           </span> */}
                           </span>
                           </div>
                 
                            {/* <div className="lg:flex hidden lg:flex-row  justify-center flex-col w-[50%] mr-8  lg:w-[15%]">
                           <span className={`border-2 p-1 border rounded-4xl ${card?.accept ? "text-[green] bg-[green]/30":"  border-[gray]/30 bg-gray-300"}  `}>
                               {card?.accept ? "تایید شده": "در انتظار تایید"}
                           </span>
                           </div> */}
       
       
                            <span className=" flex flex-row justify-between lg:w-[5%] lg:mr-4 lg:gap-0 gap-2 lg:mt-0 lg:ml-2 ml-2 mt-5 ">
                             <button onClick={()=> deleteFavNews(card?.favoriteId)}> <img className="" src={Delete} /></button>
                             <button onClick={() => setShowCourseDetail(card?.newsId)}> <img className="" src={Show} /></button>
