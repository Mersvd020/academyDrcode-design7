...
const renderTeacher = (row) => {
  const teacher = row.course.teacher || {};
...
export const columns = [
      ...
      return (
        <Fragment key={`status-${row.courseId}`}>
          <Avatar
            color={color}
            icon={<Icon size={15} />}
            id={`av-tooltip-${row.courseId}`}
          />
          <UncontrolledTooltip
            placement="top"
            target={`av-tooltip-${row.courseId}`}
          >
    ...
    cell: (row) => {
      return (
        <div className="d-flex flex-column">
          <h6 className="user-name text-truncate mb-0">{row.course?.title}</h6>
          <small className="text-truncate text-muted mb-0">
            {row.course?.miniDescribe || "بدون توضیحات"}
          </small>
        </div>
      );
    },
  ...
  },
  {
    ...
    cell: (row) => {
      const date = new Date(row.course.startTime);
        ...
        <div className="column-action d-flex align-items-center">
          ...
          <UncontrolledDropdown>
            ...
            <DropdownMenu end>
              ...
              <span className='align-middle'>ویراش دوره</span>
            </DropdownItem> */}
              <DropdownItem
                tag={Link}
                to={`/apps/course/edit/${row.courseId}`}
                className="w-100"
              >
              ...
              <DropdownItem
                ...
                onClick={(e) => {
                  e.preventDefault();
                  store.dispatch(deleteCourse(row.courseId));
                }}
              >
